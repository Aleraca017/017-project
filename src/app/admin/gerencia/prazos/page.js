...
export default function PrazosPage() {
    ...
    const getPrazoStatus = (dataEntrega) => {
        ...
        const hoje = new Date();
        const entrega = dataEntrega.toDate ? dataEntrega.toDate() : new Date(dataEntrega);
    ...
    const getStatusDisplay = (projeto) => {
        const status = projeto.status?.toLowerCase();
        if (status === "concluido")
            return { label: "Concluído", className: "bg-green-700 text-white" };
        if (status === "cancelado")
            return { label: "Cancelado", className: "bg-red-700 text-white" };
        if (status === "pendente")
            return { label: "Pendente", className: "bg-gray-500 text-white" };

        // em andamento → usa lógica do prazo
        if (status === "em andamento")
            return getPrazoStatus(projeto.dataEntrega);
                ...
                <main className="flex-1 p-6">
                    ...
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
                        ...
                        <input
                            ...
                            className="shadow-lg w-90 shadow-purple-950 focus:shadow-purple-700 rounded px-3 py-2 ring-2 ring-purple-600 outline-none placeholder-purple-400 text-gray-50"
                        />
                    </div>
                                ...
                                {filteredProjects.map((p, index) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <td className="p-3 text-zinc-50">
                                                {p.dataEntrega ? format(p.dataEntrega.toDate ? p.dataEntrega.toDate() : new Date(p.dataEntrega), "dd/MM/yyyy") : "-"}
                                            </td>
                                            <td className="p-3">
                                                {statusDisplay && (
                                                    <span
                                                        className={`px-3 py-1 rounded-full text-sm font-semibold ${statusDisplay.className}`}
                                                    >
                                                        {statusDisplay.label}
                                                    </span>
                                                )}
